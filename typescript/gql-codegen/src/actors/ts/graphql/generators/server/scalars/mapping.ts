...
export type ScalarsMapping = Record<BuiltinScalarName, ScalarSpec> &
    Record<string, ScalarSpec>;

export function buildSymmetricScalarSpec(specType: ts.Expression): ScalarSpec {
    return {
...
export function getScalarSpecFromMapping(
    ...
    if (!(name in scalarsMapping)) {
        throw new Error(
            `No scalar spec defined in mapping for scalar: ${name}`,
        );
    }
