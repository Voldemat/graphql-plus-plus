...
>() {
    return async <T extends SubscriptionOperation<unknown, unknown>>(
        ...
        const signal = new Promise<ReadResult>((res) => (resolve = res));
        const close = () => {
            shouldClose = true;
        ...
        const stream = (async function* () {
            while (!shouldClose) {
                const readResult = await Promise.race([reader.read(), signal]);
                if (readResult.done) return;
                const lines = decoder.decode(readResult.value).split('\n');
