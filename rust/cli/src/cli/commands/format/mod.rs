...
fn format_check_action(
    ...
    match crate::cli::text_diff::get_diff_string(
        initial_buffer,
        &formatted_string,
    ) {
        None => Ok(()),
    ...
    pub fn execute(self: &Self) {
        ...
        if let Some(formatting_server_config) = formatting_config.server {
            ...
            for graphql_path in crate::cli::utils::resolve_paths(
                ...
                if self.check {
                    ...
                    if let Err(error) = format_action(
                        &graphql_path,
                        &lir_printer_config,
                        lir_nodes,
                    ) {
        ...
        if let Some(formatting_client_config) = formatting_config.client {
            ...
            for graphql_path in crate::cli::utils::resolve_paths(
                ...
                let buffer = std::fs::read_to_string(&graphql_path).unwrap();
                let ast_nodes =
                    match buffer_to_client_ast(&graphql_path, &buffer) {
                        Ok(nodes) => nodes,
                        Err(error) => {
                            errors.push(error);
                            continue;
